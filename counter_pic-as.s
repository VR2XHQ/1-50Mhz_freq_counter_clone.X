; Beny 2025-08-28
; Convert the original MPASM code for comiling with XC8 pic-as.
; Keep original comments as far as possible.
...
;**************************************************************************
 ...
 ; Beny: Manually define which variant to go.
...
; Beny: New syntax for pic-as.
  CONFIG  FOSC = XT             ; Oscillator Selection bits (XT oscillator: Crystal/resonator on RA6/OSC2/CLKOUT and RA7/OSC1/CLKIN)
  CONFIG  WDTE = ON             ; Watchdog Timer Enable bit (WDT enabled)
  CONFIG  PWRTE = ON            ; Power-up Timer Enable bit (PWRT enabled)
  CONFIG  MCLRE = OFF           ; RA5/MCLR pin function select (RA5/MCLR pin function is digital input, MCLR internally tied to VDD)
...
;   __IDLOCS H'1234'
; Beny: Just show how to set ID number with pic-as syntax.
CONFIG IDLOC0 = 0x42
CONFIG IDLOC1 = 0x01
CONFIG IDLOC2 = 0x25
CONFIG IDLOC3 = 0x7F
...
PORT_A_IO      equ  0000B        ; port A I/O mode (all output)
PORT_B_IO      equ  00000000B    ; port B I/O mode (all output)
...
eep_dw  macro value  ; a DOUBLEWORD split into 4 bytes in the PIC's DATA EEPROM
        ...
        endm
; Beny: Change from 'de','&' to 'db',"and" for pic-as.
...
; Beny: start from EEPROM_ADR_STD_IF_TABLE 
    ...
    eep_dw 10695000   ; [18..1F] reserved for other "preprogrammed" values
		eep_dw 12345678		; Just for fun!

; org (0x2100+EEPROM_ADR_OPTIONS)
; Beny: start from EEPROM_ADR_OPTIONS
    db      0         ; [20]     "options" (flags), cleared by default
; Beny: Change from 'de' to 'db' for pic-as.
...
; Beny: This part is adapted from Aniello Di Nardo for easy coversion.
;**************************************************************************
;                                                                         *
...
cquad  macro value
       ...
       endm
; Beny: Change from '&' to "and" for pic-as.

; Beny: Below do not use in orginal code.
...
BLANK_PATTERN equ 00000000B      ; blank display pattern (7-segment code)
...
Digit2SevenSeg:
 ...
 ; for COMMON CATHODE: No bitwise EXOR to the pattern
          ...
          retlw (11111111B        )^SSEG_XORMASK ; all segments on  ( # 20 )
...
; muliplexer values for DISPLAY VARIANT #1 :
					retlw 11110111B        ; most significant digit is on   PA3 (!)
          retlw 11111110B        ; next less significant dig. on  PA0 (!)
          retlw 11111011B        ; next less significant dig. on  PA2 (!)
          retlw 11111101B        ; 4th (sometimes the last) digit PA1 (!)
          retlw 11111111B        ; 5th (OPTIONAL) least significant digit = NOT (PA3+PA2+PA1+PA0)
#endif
; DISPLAY VARIANT #1
#if (DISP_VARIANT==2)
...
; muliplexer values for DISPLAY VARIANT #3 (5 digits, COMMON ANODE) :
					...
					retlw 11111000B        ; most significant digit is on   PA3 (!)
          retlw 11110100B        ; next less significant dig. on  PA2 (!!)
          retlw 11110001B        ; next less significant dig. on  PA0 (!!)
          retlw 11110010B        ; 4th (sometimes the last) digit PA1 (!)
          retlw 11110000B        ; 5th (OPTIONAL) least significant digit = NOT (PA3+PA2+PA1+PA0)
#endif
; DISPLAY VARIANT #2
...
PSC_DIV_BY_2   equ  00100000B   ; let prescaler divide TMR0 by two
PSC_DIV_BY_4   equ  00100001B   ; let prescaler divide TMR0 by   4
PSC_DIV_BY_8   equ  00100010B   ; let prescaler divide TMR0 by   8
PSC_DIV_BY_16  equ  00100011B   ; let prescaler divide TMR0 by  16
PSC_DIV_BY_32  equ  00100100B   ; let prescaler divide TMR0 by  32
PSC_DIV_BY_64  equ  00100101B   ; let prescaler divide TMR0 by  64
PSC_DIV_BY_128 equ  00100110B   ; let prescaler divide TMR0 by 128
PSC_DIV_BY_256 equ  00100111B   ; let prescaler divide TMR0 by 256
...
SetPrescaler:  ; copy W into OPTION register, avoid watchdog trouble
          ...
          retlw 0
;Beny: Some syntax changed for pic-as. Refer the comment out original code.
;Beny: 'movfw F' is changed to 'movf F,w'
;Beny: Those apply to the rest of the code.
...
PrescalerOff:  ; turn the prescaler for TMR0 "off" 
          ...
          bsf STATUS, STATUS_RP0_POSITION
					;bsf   STATUS, RP0            ;! setting RP0 enables access to OPTION reg
               ; option register is in bank1. i know. thanks for the warning.
          movlw 00100111B            ;! recommended by Microchip when
                                       ;! changing prescaler assignment from TMR0 to WDT
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          clrwdt                       ;! clear watchdog again
          movlw 00101111B            ;! bit 3 set means PS assigned to WDT now
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          bcf STATUS, STATUS_RP0_POSITION
					;bcf   STATUS, RP0            ;! clearing RP0 for normal register access
          ;errorlevel +302 ; Enable banking message again
...
; not COMMON CATHODE but COMMON ANODE:
          ...
          bsf STATUS, STATUS_RP0_POSITION
					;bsf   STATUS, RP0            ;! setting RP0 enables access to OPTION reg
               ; option register is in bank1. i know. thanks for the warning.
          movlw 00101011B            ;! assign PS to WDT; divide by 8 FOR WDT(!)
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          bsf STATUS, STATUS_RP0_POSITION          
					;bcf   STATUS, RP0            ;! clearing RP0 for normal register access
          ;errorlevel +302 ; Enable banking message again
...
SaveEW:  btfsc   EECON1, EECON1_WR_POSITION          ;!; WR is cleared after completion of write
...
PSECT code
MainInit:
;Beny: Entry point must also use PSECT for declaration in pic-as.

; Test some math macros ?
;#IF 0 ;Beny: no capital letter.
#if 0
				clrf  freq2_hi
          clrf  freq2_mh
          clrf  freq2_ml
...
;; Test !
          ...
          ;errorlevel -302 ; Turn off banking message for the next few instructions..
          bsf   STATUS, STATUS_RP0_POSITION            ;! setting RP0 enables access to TRIS regs
          movwf PORTA                  ;! looks like PORTA but is in fact TRISA 
          bcf   STATUS, STATUS_RP0_POSITION            ;! clearing RP0 enables access to PORTs
          clrf PORTA
...
MainLoop:
          ...
          movlw 100000B               ; value for OPTION reg: edge - low-to-high transition, 
                                        ;  + prescaler assigned to Timer 0, 1:2
          bsf   STATUS, STATUS_RP0_POSITION            ;! setting RP0 enables access to OPTION reg
               ; option register is in bank1. i know. thanks for the warning.
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          bcf   STATUS, STATUS_RP0_POSITION            ;! clearing RP0 for normal register access
          ;errorlevel +302 ; Enable banking message again
...
; end TEST
          ...
          andlw 11000000B    ; any of bits 15..14 set (>=16384) -> no Z flag -> range 9
...
; end of specific range-switching for  DISPLAY VARIANT #1
          ...
          andlw 11000000B    ; any of bits 15..14 set (>=16384) -> no Z flag -> range 7
...
;Beny: Must set the resetVec here for pic-as.
