...
; 2011-06-12 - Added external prescaler support with selectable           *
;              prescaling ratio (Klima)                                   *
...
MI_PRESC  equ  6      ; jump into the prescaler setting
MI_INDEX_MAX  equ 6  ; normal menu indices up to MI_INDEX_MAX .
MI_IF_1   equ  7     ; show the 1st standard IF
MI_IF_2   equ  8     ; show the 2nd standard IF
MI_IF_3   equ  9     ; show the 3rd standard IF
MI_IF_4   equ  0x0A  ; show the 4th standard IF
MI_IF_5   equ  0x0B  ; show the 5th standard IF
MI_IF_QT  equ  0x0C  ; exit standard IF menu without changing anything
MI_IF_SUBMENU_MAX equ 0x0C
MI_PS_1   equ  0x0D     ; show the 1st prescaler setting
MI_PS_2   equ  0x0E     ; show the 2nd prescaler setting
MI_PS_3   equ  0x0F     ; show the 3rd prescaler setting
MI_PS_4   equ  0x10     ; show the 4th prescaler setting
MI_PS_5   equ  0x11     ; show the 5th prescaler setting
MI_PS_QT  equ  0x12     ; exit standard prescaler menu without changing anything
MI_PS_SUBMENU_MAX equ 0x12
...
prescaler      equ 0x57 ;current prescaler
...
#define EEPROM_ADR_STD_IF_TABLE 0x04  ; EEPROM location for standard IF table (5*4 byte)
#define EEPROM_ADR_OPTIONS      0x20  ; EEPROM location for "options" (flags)
#define EEPROM_ADR_STD_PS_TABLE 0x24  ; EEPROM location for standard Prescaler table (5*4 byte)
...
; Initial contents of DATA EEPROM:
 ...
 org (0x2100+2*EEPROM_ADR_STD_PS_TABLE)  ;  standard IF table ...
    ...
    eep_dw  .64         ; [30..33] external prescaler by 64
...
TensTable  addwf PCL,f  
           cquad   .100000000  ; 100 million is better
...
PMDisplay:
          ...
          goto   PmDisp_Presc ; prescaler setting
          ...
          goto   PmDisp_PS_1 ; show 1st standard PS from table
          goto   PmDisp_PS_2 ; show 2nd standard PS from table
          goto   PmDisp_PS_3 ; show 3rd standard PS from table
          goto   PmDisp_PS_4 ; show 4th standard PS from table
          goto   PmDisp_PS_5 ; show 5th standard PS from table
          goto   PmDisp_Quit ; show "quit" (quit STANDARD PS menu)
...
PMExecute:   ; Execute the function belonging to menu_index
          ...
          goto   PmExec_Presc ; select prescaler
          ...
          goto   PmExec_SelPS ; select 1st standard IF from table
          goto   PmExec_SelPS ; select 2nd standard IF from table
          goto   PmExec_SelPS ; select 3rd standard IF from table
          goto   PmExec_SelPS ; select 4th standard IF from table
          goto   PmExec_SelPS ; select 5th standard IF from table
          goto   PmExec_Quit ; quit STANDARD PS menu
...
MainRestart:  ; Here we "restart" the counter after exiting from programming mode :
          ...
          movlw  prescaler                ; another destination address for reading from EEPROM..
          movwf  FSR                      ;
          movlw  EEPROM_ADR_PRESCALER     ; load EEPROM-internal offset of prescaler
          call   EEPROM_ReadByte          ; read single byte from EEPROM: prescaler := EEEPROM[W]
...
PmDisp_Presc: ; select prescaler setting
          movlw  CHAR_P
          call   conv_char0
          movlw  CHAR_r
          call   conv_char1
          ...
          call   conv_char4
          goto   ProgModeDisplay
PmDisp_PS_1: ; show 1st standard IF from table
          movlw  EEPROM_ADR_STD_PS_TABLE + 4*0
          goto   PmLoadFreq2
PmDisp_PS_2: ; show 2nd standard IF from table
          movlw  EEPROM_ADR_STD_PS_TABLE + 4*1
          goto   PmLoadFreq2
PmDisp_PS_3: ; show 3rd standard IF from table
          movlw  EEPROM_ADR_STD_PS_TABLE + 4*2
          goto   PmLoadFreq2
PmDisp_PS_4: ; show 4th standard IF from table
          movlw  EEPROM_ADR_STD_PS_TABLE + 4*3
          goto   PmLoadFreq2
PmDisp_PS_5: ; show 5th standard IF from table
          movlw  EEPROM_ADR_STD_PS_TABLE + 4*4
          goto   PmLoadFreq2
...
PmExec_Presc   ; switch to  prescaler selection
          movlw  MI_PS_1
PmExec_SetPS: movwf  menu_index
          goto   ProgLoop       ;

PmExec_SelPS  ; Finished selecting a "standard PS" from table.
;          movlw  PS_SELECT         ; Suggestion: ADD the offset
;          goto   PmExec_SetPS

SavePS2:   ; save <freq2> (1 byte) in the PIC's EEPROM memory :
          movlw  freq2+3          ; load the ADDRESS of 'freq2' last byte...
          movwf  FSR            ; ... into the PIC's "pointer" register
          movlw  EEPROM_ADR_PRESCALER ; load the EEPROM-internal address offset (=destination)
          call   SaveInEEPROM   ; write *FSR into EEPROM[w]     (bits 31..24)
...
NotIfSubMenu:   ; was not in the "standard IF submenu"..
          ...
          movfw  menu_index
          sublw  MI_PS_SUBMENU_MAX ; subtract #MI_.. - W register -> C=0 if result negative ("W too large")
          btfsc  STATUS,Z        ; skip next instruction if Z=0
          goto   LastPsSubMenu   ; Z=1 means "this is the last item in the main menu"
          btfss  STATUS,C        ; skip next instruction if C=1
          goto   NotAnySubMenu    ; C=0 means "this is not the main menu"
          incf   menu_index, f   ; menu_index := menu_index+1  (in submenu)
          goto   ProgLoop        ;
LastPsSubMenu:  ; was in the last "standard IF submenu"..
          movlw  MI_PS_1         ; back to the 1st standard IF submenu
          movwf  menu_index
          goto   ProgLoop  
