...
; EX(16F84:)     __CONFIG   _CP_OFF & _WDT_ON & _PWRTE_ON & _RC_OSC
   ...
   __CONFIG   _CP_OFF & _WDT_ON & _PWRTE_ON & _HS_OSC & _LVP_OFF & _BODEN_OFF & _CPD_OFF & _WRT_OFF
...
PORT_A_IO      equ  b'11010000'        ; port A I/O mode (all output)
PORT_D_IO      equ  b'00000000'    ; port D I/O mode (all output)

LEDS_PORT      equ  PORTD          ; 7-segment LEDs port
...
#define IOP_PROG_MODE  PORTB,1   ; digital input signal, LOW enters programming mode
...
;#define _G  0x80
;#define _DP 0x10
DPPOINT_BIT equ  7   ; decimal point bit (same for all digits)
#define _A  0x01     ; bitmask for segment A , etc ..
#define _B  0x02
#define _C  0x04
#define _D  0x08
...
Digit2MuxValue:     ; 
          ...
          retlw b'00110000'        ; most significant digit is on   PA3 (!)
          retlw b'00011000'        ; next less significant dig. on  PA2 (!!)
          retlw b'00010100'        ; next less significant dig. on  PA0 (!!)
          retlw b'00010010'        ; 4th (sometimes the last) digit PA1 (!)
          retlw b'00010001'        ; 5th (OPTIONAL) least significant digit = NOT (PA3+PA2+PA1+PA0)#endif   ; DISPLAY VARIANT #2
#endif   ; DISPLAY VARIANT #1
...
PSC_DIV_BY_2   equ  b'00110000'   ; let prescaler divide TMR0 by two
PSC_DIV_BY_4   equ  b'00110001'   ; let prescaler divide TMR0 by   4
PSC_DIV_BY_8   equ  b'00110010'   ; let prescaler divide TMR0 by   8
PSC_DIV_BY_16  equ  b'00110011'   ; let prescaler divide TMR0 by  16
PSC_DIV_BY_32  equ  b'00110100'   ; let prescaler divide TMR0 by  32
PSC_DIV_BY_64  equ  b'00110101'   ; let prescaler divide TMR0 by  64
PSC_DIV_BY_128 equ  b'00110110'   ; let prescaler divide TMR0 by 128
PSC_DIV_BY_256 equ  b'00110111'   ; let prescaler divide TMR0 by 256
...
PrescalerOff:  ; turn the prescaler for TMR0 "off" 
          ...
          bsf   STATUS, RP0            ;! setting RP0 enables access to OPTION reg
               ; option register is in bank1. i know. thanks for the warning.
          movlw b'00110111'            ;! recommended by Microchip when
                                       ;! changing prescaler assignment from TMR0 to WDT
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          clrwdt                       ;! clear watchdog again
          movlw b'00111111'            ;! bit 3 set means PS assigned to WDT now
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          bcf   STATUS, RP0            ;! clearing RP0 for normal register access
          errorlevel +302 ; Enable banking message again
          retlw 0
...
Sleep150ms:  ; go to sleep for approx. 150 milliseconds, and then RETURN (no reset)
   ...
   ; > a Watchdog Timer timeout.
          ...
          bsf   STATUS, RP0            ;! setting RP0 enables access to OPTION reg
               ; option register is in bank1. i know. thanks for the warning.
          movlw b'00111011'            ;! assign PS to WDT; divide by 8 FOR WDT(!)
          movwf OPTION_REG             ;! ex: "option" command (yucc)
          bcf   STATUS, RP0            ;! clearing RP0 for normal register access
          errorlevel +302 ; Enable banking message again
...
#ENDIF  ; Test !
         ...
         errorlevel +302
          ...
          movlw PORT_D_IO               ; initialise port D
          bsf   STATUS, RP0            ;! setting RP0 enables access to TRIS regs
          movwf PORTD                  ;! looks like PORTD but is in fact TRISD
          bcf   STATUS, RP0            ;! clearing RP0 enables access to PORTs
          errorlevel +302 ; Enable banking message again
          clrf  PORTD
...
MainLoop:
          ...
          movlw b'00110000'               ; value for OPTION reg: edge - low-to-high transition, 
